...
// Extract Merchant ID (assuming it's the first part of Client ID or the Client ID itself)
const MERCHANT_ID = "M23KE745L38NT";
...
async function runTest() {
    try {
        ...
        console.log(`Testing PAY_PAGE with merchantId: ${MERCHANT_ID}`);

        const payload = {
            merchantId: MERCHANT_ID, // ADDED THIS
        ...
        const url = response.data.data?.instrumentResponse?.redirectInfo?.url || response.data.redirectUrl;
        console.log("\nGenerated URL:", url);
